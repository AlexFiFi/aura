...
namespace Aura.Login.Util
{
	public static class LoginConf
	{
		...
		public static void Load(string[] args)
		{
			...
			LoginConf.ConsoleFilter = (LogLevel)_conf.GetInt("login_consolefilter", 0);
#if DEBUG
		...
		public static void LoadRound2()
		{
			...
			if (spawn.Length == 3)
			{
				...
				if (!uint.TryParse(spawn[0], out region))
				{
					...
					if (region == 0)
						Logger.Warning("login.conf, login_spawn : Map '{0}' not found.", spawn[0]);
				}
				...
				if (region > 0)
				{
					...
					if (!uint.TryParse(spawn[1], out LoginConf.SpawnX))
						Logger.Warning("login.conf, login_spawn : Invalid format for 'X'.");
					if (!uint.TryParse(spawn[2], out LoginConf.SpawnY))
						Logger.Warning("login.conf, login_spawn : Invalid format for 'Y'.");
				}
			}
		}
	}
